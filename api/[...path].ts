...
function buildPaletteColors(palette: string) {
  ...
  return {
    primary: colors[0] || "#111827",
    secondary: colors[1] || "#F97316",
    accent: colors[2] || "#FDE047",
...
function buildPreviewCard(params: {
  ...
  const titleBlock = renderSvgTextBlock({
    ...
    lines: titleLines,
    fontSize: variant === "cover" ? 108 : variant === "cta" ? 78 : 68,
    lineHeight: variant === "cover" ? 98 : 64,
    fill: "#F8FAFC",
  ...
  const layouts: Record<StylePreset, Record<"cover" | "content" | "cta", string>> = {
    bold: {
      cover: `
